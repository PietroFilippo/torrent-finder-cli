...
- **Multi-Engine Fan-Out:** Each provider queries its enabled sources in parallel and merges results, deduplicating by info hash and sorting by seeders. Movies & Series enables Apibay + Nyaa live-action by default, with SolidTorrents and YTS available as optional engine toggles; Anime uses Nyaa, Manga combines Nyaa Literature + Apibay Comics, and Books pairs Libgen with Apibay E-books/Audio books — keeping Libgen rows above the torrent rows, since direct downloads have no seeder count to sort by.
