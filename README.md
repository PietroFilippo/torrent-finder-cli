...
- **Multi-Engine Fan-Out:** Each provider queries its **On** sources in parallel and merges results, deduplicating by info hash and sorting by seeders. If they all return zero raw rows, a category-scoped **Auto** Knaben search runs; **Off** engines are never contacted. Movies & Series keeps Apibay + Nyaa live-action On, Anime uses Nyaa, Manga combines Nyaa Literature + Apibay Comics, and Books pairs Libgen with Apibay E-books/Audio books. YTS and SolidTorrents remain available as manual Off-by-default options.
