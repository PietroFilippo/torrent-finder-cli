...
- **Multi-Engine Fan-Out:** Each provider queries its enabled sources in parallel and merges results, deduplicating by info hash and sorting by seeders. Movies & Series enables Apibay + Nyaa live-action by default, with SolidTorrents and YTS available as optional engine toggles; Anime uses Nyaa, while Manga combines Nyaa Literature + Apibay Comics.
...
- **Search prompt (multi-title)**: `Ctrl+F` jumps to filters, `Ctrl+N` commits the current title and starts another line, and `Tab` opens quick actions. `Enter` searches all entered titles at once; `Up`/`Down` recall past searches.
